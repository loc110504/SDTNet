...
parser.add_argument('--exp', type=str,
                    default='TABNet_no_Intensity', help='experiment_name')
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled in enumerate(trainloader):
            ...
            loss_TAS = loss_ce_i + loss_ce_j
            # BAP
            with torch.no_grad():
                denom = (loss_ce_i.detach() + loss_ce_j.detach()).clamp_min(1e-8)
                w_i = (loss_ce_j.detach() / denom).item()
                w_j = (loss_ce_i.detach() / denom).item()
            mixed_prob = w_i * y_i + w_j * y_j
            y_pl = torch.argmax(mixed_prob.detach(), dim=1)  
            loss_PL = dice_loss(y_i, y_pl.unsqueeze(1)) + dice_loss(y_j, y_pl.unsqueeze(1))
            ...
            loss_BD = bd_loss_fn(B_i, B_pl.detach()) + bd_loss_fn(B_j, B_pl.detach())
